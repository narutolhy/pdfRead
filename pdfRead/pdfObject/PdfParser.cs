...
//		new OpCtrl("ID", Operator.BeginInlineImageData),
        ...
        static PdfParser() {
            ...
            foreach(OpCtrl Op in OpCtrlArray)
                if(Op.OpStr != "F")
                    OpStr[(Int32)Op.OpCode] = Op.OpStr;
            return;
        }
        ...
        public static Boolean IsDelimiter(Char Ch) {
            return (Delimiter[(Int32)Ch] != 0);
        }
        public static Boolean IsDelimiter(Byte Ch) {
            return (Delimiter[(Int32)Ch] != 0);
        }
        public static Boolean IsDelimiter(Int32 Ch) {
            return (Delimiter[Ch] != 0);
        }
        public static Boolean IsWhiteSpace(Char Ch) {
            return ((Delimiter[(Int32)Ch] & 2) != 0);
        }
        public static Boolean IsWhiteSpace(Byte Ch) {
            return ((Delimiter[(Int32)Ch] & 2) != 0);
        }
        public static Boolean IsWhiteSpace(Int32 Ch) {
            return ((Delimiter[Ch] & 2) != 0);
        }
        ...
        public PdfBase ParseNextItem() {
            ...
            if(Single.TryParse(NextItem.ToString(),
                NumberStyles.AllowDecimalPoint | NumberStyles.AllowLeadingSign, NFI.DecSep, out RealVal))
                return (new PdfReal(RealVal));

            // false
            if(NextItem.ToString() == "false")
                return (new PdfBoolean(false));

            // true
            if(NextItem.ToString() == "true")
                return (new PdfBoolean(true));

            // null
            if(NextItem.ToString() == "null")
                return (new PdfNull());

            // parse page contents
            if(StreamMode) {
                // begin inline image
                if(NextItem.ToString() == "BI")
        ...
        public void SkipWhiteSpace() {
            ...
            if(IsWhiteSpace(NextChar))
                while((NextChar = ReadChar()) != EOF && IsWhiteSpace(NextChar))
                    ;
            return;
        }
        ...
        private Int32 TestReference() {
            ...
            if(TempChar != 'R' && (TempChar != 'o' || ReadChar() != 'b' || ReadChar() != 'j'))
                goto not_found;
        ...
        private PdfBase ParseString() {
            ...
            for(;;) {
                ...
                NextChar = ReadChar();
                if(NextChar == EOF)
                    throw new ApplicationException("Invalid string (End of contents)");
        ...
        private PdfBase ParseDictionary() {
            ...
            for(;;) {
                ...
                if(Index >= 0)
                    throw new ApplicationException("Invalid dictionary (duplicate keys)");
            ...
            if(NextChar == EOF || NextChar != '>')
                throw new ApplicationException("Invalid dictionary (missing terminating >>");
        ...
        private PdfBase ParseInlineImage() {
            ...
            for(;;) {
                ...
                Int32 Index = ResultDict.BinarySearch(Pair);
                if(Index >= 0)
                    throw new ApplicationException("Invalid inline image (duplicate keys)");
