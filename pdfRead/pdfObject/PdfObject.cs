...
namespace pdfRead.pdfObject {
    ...
    public class PdfIndirectObject : PdfBase, IComparable<PdfIndirectObject> {
        ...
        public void ReadObject() {
            ...
            if(!ObjectValue.IsEmpty || ParentObjectNo != 0)
                return;
            ...
            if(Document.ParseNextItem().ToObjectNo != ObjectNo)
            ...
            if(ObjectValue.IsDictionary) {
                ...
                if(KeyWord == KeyWord.Stream)
                    ObjectValue = new PdfStream(ObjectValue.ToDictionary, Document.GetFilePosition());
            ...
            // object is not a dictionary
                ...
                // test for endobj 
                if(Document.ParseNextItem().ToKeyWord != KeyWord.EndObj)
                    throw new ApplicationException("'endobj' token is missing");
            }
        ...
        public void ReadStream() {
            // already done
            if(ObjectValue.StreamContents != null)
                return;
            ...
            if(ObjectValue.StreamDictionary.GetArrayOfNames("/Filter", out FilterNames)) {
                ...
                for(Int32 Index = 0; Index < FilterNames.Length; Index++) {
                    ...
                    if(FilterName == "/FlateDecode") {
                        ...
                        // save as a JPEG file
                        //Byte[] TempContents = ObjectValue.StreamContents;
                        //SaveJpg();
                        //Saved = true;
                        break;
                    } else {
                        // unsupported filter
                        throw new ApplicationException("Unsupported stream filter: " + FilterName);
            ...
            // test for endstream 
            if(Document.ParseNextItem().ToKeyWord != KeyWord.EndStream)
        ...
        public void SaveStreamContents() {
            ...
            if(BitsPerComponent == 8 && ImageWidth > 0 && ImageHeight > 0 && ColorSpace != null &&
                ...
                if(ColorSpace == "/DeviceRGB")
                    SaveRGBBitmap(ImageWidth, ImageHeight);
                else
                    SaveGrayBitmap(ImageWidth, ImageHeight);
                return;
            }
            ...
            // all others
                ...
                Int32 NonText = 0;
                foreach(Byte Chr in ObjectValue.StreamContents)
                    if(Chr < ' ' && !PdfParser.IsWhiteSpace(Chr) || Chr > '~')
                        NonText++;
        ...
        public void ParsePageContents() {
            ...
            for(;;) {
                ...
                for(;;) {
                    ...
                    if(Token.IsOperator || Token.IsInlineImage || Token.IsEmpty)
                        break;
                    ArgStack.Add(Token);
                }
            ...
            // save contents to the disk
            using(StreamWriter PrintFile = new StreamWriter(FullName))
                PrintFile.Write(Text.ToString());

            // save in Byte array
            PageSource = new Byte[Text.Length];
            for(Int32 Index = 0; Index < Text.Length; Index++)
                PageSource[Index] = (Byte)Text[Index];
        ...
        public override String ToString() {
            ...
            if(!String.IsNullOrEmpty(ObjectSubtype))
                TextFile.AppendFormat("%% Object Subtype: {0}\n", ObjectSubtype);
        ...
        public Byte[] PredictorDecode
            ...
            Int32 Predictor;
            if(!DecodeParms.GetValue("/Predictor").GetInteger(out Predictor) || Predictor == 1)
                return (InputBuffer);

            // we only support predictor code 12
            if(Predictor != 12)
                return (null); // throw new ApplicationException("/DecodeParms /Predictor is not 12");
        ...
        public Byte[] Ascii85Decode
            ...
            for(Int32 Index = 0; Index < InputBuffer.Length; Index++) {
                ...
                // ignore white space
                if(PdfParser.IsWhiteSpace(NextChar))
                    continue;
            ...
            if(State != 0) {
                ...
                // add padding of 84
                for(Int32 PadState = State; PadState < 5; PadState++)
                    FourBytes += Power85[PadState] * (UInt32)('u' - '!');
